...
st.set_page_config(page_title="🌍🍃 Green Trails", layout="wide")
st.markdown(
    """
    <style>
        .title {
...
# Title and subtitle
st.markdown('<h1 class="title">🌍🍃 Green Trails</h1>', unsafe_allow_html=True)
...
SERPAPI_KEY ="2c0e4b99f99e1ca39e19fd8ae5e281eed668376aa6a8546c01e23149a93124ad"## "07a494258a0c8899f2979ad18e146532f12d645c08e24804e8fbaf4039bf8255"
GOOGLE_API_KEY = "AIzaSyA1dTqTkvS8k9GrecevjWEfIKmI2646m58"
...
# Function to safely run AI agents with error handling
def safe_run_agent(agent, prompt, max_retries=3, fallback_message=None):
    for attempt in range(max_retries):
        try:
            ...
            if attempt == max_retries - 1:  # Last attempt
                if fallback_message:
                    return type('obj', (object,), {'content': fallback_message})
                else:
                    return type('obj', (object,), {'content': f"Sorry, we couldn't process this request after {max_retries} attempts. Please try again later."})
...
if st.button("🚀 Generate Travel Plan"):
    ...
    with st.spinner("🔍 Researching best attractions & activities..."):
        ...
        research_fallback = f"Based on typical travel recommendations for {destination}, we suggest exploring popular attractions and activities aligned with your {travel_theme.lower()} preferences. Consider local cuisine, cultural sites, and activities matching your interests in {activity_preferences}."
        research_results = safe_run_agent(researcher, research_prompt, fallback_message=research_fallback)
    ...
    with st.spinner("🏨 Searching for hotels & restaurants..."):
        ...
        hotel_fallback = f"For your {budget} budget and {hotel_rating} preference, we recommend checking popular booking sites for accommodations in {destination}. For dining, look for well-reviewed restaurants near your hotel or major attractions that match your preferences."
        hotel_restaurant_results = safe_run_agent(hotel_restaurant_finder, hotel_restaurant_prompt, fallback_message=hotel_fallback)
    ...
    with st.spinner("🗺️ Creating your personalized itinerary..."):
        ...
        itinerary_fallback = f"Here's a suggested {num_days}-day itinerary for your trip to {destination}:\n\n" + "\n".join([f"Day {i+1}: Morning - Explore local attractions, Afternoon - Enjoy activities related to {activity_preferences}, Evening - Dinner at a local restaurant" for i in range(num_days)])
        itinerary = safe_run_agent(planner, planning_prompt, fallback_message=itinerary_fallback)
...
st.sidebar.markdown("---")  # Divider for aesthetics
if st.sidebar.button("💬 AI Agent Chatbot"):
    chatbot_url = "http://127.0.0.1:5000/"  # URL where Flask app is running
    st.sidebar.markdown(
        f'<a href="{chatbot_url}" target="_blank" style="text-decoration: none;">'
        f'<button style="background-color: #2C7DA0; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer;">'
        f'Open Chatbot</button></a>',
        unsafe_allow_html=True
    )
    # Add a button to open the ride-sharing app
st.sidebar.markdown("---")  # Divider for aesthetics
if st.sidebar.button("🚗 Ride Sharing"):
    ride_sharing_url = "http://127.0.0.1:5001/"  # URL where the Flask ride-sharing app is running
    st.sidebar.markdown(
        f'<a href="{ride_sharing_url}" target="_blank" style="text-decoration: none;">'
        f'<button style="background-color: #2C7DA0; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer;">'
        f'Open Ride Sharing</button></a>',
        unsafe_allow_html=True
    )
    st.sidebar.markdown("---")  # Divider for aesthetics
if st.sidebar.button(" 🏠Renting "):
    Renting_url = "http://127.0.0.1:5002/"  # URL where the Flask ride-sharing app is running
    st.sidebar.markdown(
        f'<a href="{Renting_url}" target="_blank" style="text-decoration: none;">'
        f'<button style="background-color: #2C7DA0; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer;">'
        f'Open 🏠Renting</button></a>',
        unsafe_allow_html=True
    )